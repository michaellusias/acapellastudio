...
//
// Combines two open Feasibility Study items:
// 1. CPU/timing stability under REAL DSP load (not idle passthrough like the
//    earlier endurance test) - YIN runs every single 128-sample callback.
// 2. Real pitch detection on ACTUAL SINGING VOICE, not synthetic sine waves.
//
// DESIGN NOTE: YIN needs ~2048 samples to resolve low pitches well, but our
// real-time buffer is only 128 samples/callback. This uses a sliding ring
// buffer (append 128 new samples, drop 128 oldest, run YIN on the resulting
// 2048-sample window) every callback - a standard real-time pitch-tracking
// approach. This is a prototype; a production implementation would need a
// more rigorous lock-free ring buffer for full NFR-RT-005 compliance.
...
fn main() {
    ...
    let stream = input_device.build_input_stream(
        ...
        move |data: &[f32], _: &cpal::InputCallbackInfo| {
            ...
            // Timing gap tracking (same proxy as the earlier endurance test)
            ...
            // Update ring buffer: shift out oldest samples, append new (mono-mix if stereo)
            ...
            // Run YIN on the full window every callback - this is the real DSP load test
            if let Some(freq) = yin_detect(&buf, sample_rate, YIN_THRESHOLD) {
                if freq > 60.0 && freq < 1200.0 { // plausible vocal range, filter obvious noise
