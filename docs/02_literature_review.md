...
3. ~~No dedicated a capella-specific harmonization literature found.~~ **Corrected in Feasibility Study §6.2:** real a capella multitrack audio datasets (JaCappella, Dagstuhl ChoirSet) were subsequently found — see `03_feasibility_study.md` for details. Genre/scale mismatch with our target use case remains a real limitation, but the flat absence claimed here was outdated as soon as dataset-focused (rather than harmonization-technique-focused) search terms were used.
